...
class XMPopupListView: UIControl {
    ...
    var tableView: UITableView = {
        ...
        tb.layer.borderWidth = 1.0
        tb.layer.borderColor = Constants.Colors.lightBorderColor_forCollectionCELLS.cgColor
...
//        // SAAD's work START
        ...
        // tableView.backgroundColor = Constants.Colors.navigationBar_tintColor
        
       // tableView.dropShadow(color: .lightGray, opacity: 1, offSet: CGSize(width: -1, height: 1), radius: 5, scale: true)
       // tableView.backgroundColor = UIColor.white
        
//        let h = screenBounds.height/2
//        let w = screenBounds.width/1.5
//        let x = (screenBounds.width - w)/2
//        let y = (screenBounds.height - h)/2
//
//        self.boundView?.frame.origin.x = x
//        self.boundView?.frame.origin.y = y
//        self.boundView?.frame.size.width = w
//
//        // SAAD's work END
    ...
    func show(){
        ...
        UIView.animate(withDuration: 0.3) {
            self.tableView.frame = CGRect(x: rect.minX, y: rect.maxY + 5, width: boundViewframe.width, height: CGFloat(40 * (rows ?? 0)))
        }
...
extension XMPopupListView: UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        if self.xm_dataSource != nil
        {
            let cell = (self.xm_dataSource?.itemCell(indexPath: indexPath))!
           // cell.contentView.backgroundColor = Constants.Colors.navigationBar_tintColor
