...
class SignUpViewController: UIViewController, UserDetailDelegate, UserStatusDelegate, FBSDKLoginButtonDelegate, UpdateUser_FCM_Protocol, IsTwitter_Facebook_IdAlready_Exist_Delegate
{
    ...
    @IBAction func agreeConditionsAction(_ sender: UIButton) {
        ...
        self.performSegue(withIdentifier: Constants.Segues.SignUpVC_to_MoreUserDetails1VC, sender: self)
    }
    ...
    func setup_controls()
    {
        ...
        SignUpViewController.linkedinHelper = LinkedinSwiftHelper(configuration: config!, nativeAppChecker: nil, clients: nil, webOAuthPresent: self, persistedLSToken: nil)
    }
    ...
    @IBAction func backButtonTapped(_ sender: UIButton)
    {
        ...
        //        if(PlayList_ViewController.isComingFrom_PlayList_ViewController)
        //        {
    ...
    func LinkedIn_LogIn_Tapped()
    {
        ...
        return
//
//        SignUpViewController.linkedinHelper?.authorizeSuccess({(lsToken) -> Void in
//
...
//        })
    ...
    /*
     Utilities.show_ProgressHud(view: self.view)
     if let accessToken = UserDefaults.standard.object(forKey: "LIAccessToken") {
     let targetURLString = "https://api.linkedin.com/v1/people/~:(id,first-name,last-name,email-address,picture-url,picture-urls::(original),positions,date-of-birth,phone-numbers,location,public-profile-url)?format=json"
     let request = NSMutableURLRequest(url: NSURL(string: targetURLString)! as URL)
     request.httpMethod = "GET"
     request.addValue("Bearer \(accessToken)", forHTTPHeaderField: "Authorization")
     let session = URLSession(configuration: URLSessionConfiguration.default)
     ...
     return
     //        SignUpViewController.linkedinHelper?.requestURL(Constants.LinkedIn.requestURL, requestType: LinkedinSwiftRequestGet, success: { (response) -> Void in
     //
    ...
    deinit
    {
     ...
     {
        if(segue.identifier == Constants.Segues.signUp_to_EnterPhoneNumber_VC)
        {
            let nextVC = segue.destination as! PhoneNumberFlowViewController
            nextVC.isComingFrom_SignInVC = false
            nextVC.isComingFrom_SignUpVC = true
        }
        
        if(segue.identifier == Constants.Segues.SignUpVC_to_MoreUserDetails1VC)
        {
...
//            }
    ...
    {
        self.performSegue(withIdentifier: Constants.Segues.signUp_to_EnterPhoneNumber_VC, sender: self)
    }
}


extension SignUpViewController: MyLITokenRequestDelegate {
