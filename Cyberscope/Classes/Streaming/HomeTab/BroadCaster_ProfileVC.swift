...
class BroadCaster_ProfileVC: UIViewController, Add_Follower_Delegate, removeFollower_Delegate, Add_BlockedUser_Delegate, Remove_BlockedUser_Delegate, get_Users_Delegate, getFollowers_Data_Delegate, VideosDetailsDelegate, AddUpdateVideo_Protocol
{
    ...
    @IBAction func followerBtnN(_ sender: Any) {
        ...
        if(self.button_follow.titleLabel?.text == Constants.strings.Follow) {
            ...
            if (self.selectedBroadcaster != nil) {
                ...
                self.dataAccess.addNewFollower(follower: f, delegate: self, senderTag: (self.selectedStream?.id)!)
            }
            else {
            ...
            if let follower = Utilities.get_RequiredFollowerObject_fromFollowingList_using_followerid(id: (self.selectedStream?.username)!) {
                self.dataAccess.removeFollower(follower, resultDelegate: self, senderTag: (self.selectedStream?.id)!)
            }
        }
        else {
    ...
    @IBAction func appointmentBtnN(_ sender: Any) {
        ...
        if (self.selectedBroadcaster != nil) {
            self.performSegue(withIdentifier: Constants.Segues.BroadCaster_ProfileVC_to_Request_AppointmentVC, sender: self)
        }
        else {
        }
    }
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name("updateUserInfo"), object: nil, queue: nil){(note) in
            ...
            self.dataAccess.get_Users((self.selectedStream?.username)!, resultDelegate: self)
            Utilities.show_ProgressHud(view: self.view)
            let username = (self.selectedStream?.username)!
    ...
    func setup_Controls()
    {
        ...
        followerBtnN.layer.cornerRadius = followerBtnN.frame.height / 2
        appointmentBtnN.layer.cornerRadius = appointmentBtnN.frame.height / 2
...
//        rateViewContainer.layer.shadowRadius = 3
        ...
        self.label_broadcasterName.text = (self.selectedStream?.name)!
        //        self.label_broadcasterJobTitle.text = Constants.userSkillsType.broadcaster
        
        
        let isFollowing = Utilities.check_isCurrentUser_AlreadyFollowing_Broadcaster(broadcaster_username: (self.selectedStream?.username)!)
        if(isFollowing)
        {
        ...
        let isBlocked = Utilities.check_isBroadcaster_AlreadyBlocked(broadcaster_username: (self.selectedStream?.username)!)
        if(isBlocked)
        {
        ...
        self.view.bringSubview(toFront: button_Call)
    }
    ...
    {
        if(segue.identifier == Constants.Segues.BroadCaster_ProfileVC_to_Request_AppointmentVC)
        {
            let nextVC = segue.destination as! RequestAppointmentViewController
            nextVC.selected_broadcaster = self.selectedBroadcaster
        }
        
        if(segue.identifier == Constants.Segues.BroadcastVC_to_FollowersList_VC)
        {
            let followersVC = segue.destination as! FollowersList_VC
            followersVC.followersList = self.selectedBroadcaster_FollowersList
        }
        
        if(segue.identifier == Constants.Segues.BroadcastVC_to_FollowingsList_VC)
        {
            let followingsVC = segue.destination as! FollowingsList_VC
            followingsVC.followingsList = self.selectedBroadcaster_FollowingsList
        }
        if(segue.identifier == Constants.Segues.BroadcasterVC_to_Rating_VC)
        {
    ...
    @IBAction func btnViewRating_pressed(_ sender: Any) {
        self.performSegue(withIdentifier: Constants.Segues.BroadcasterVC_to_Rating_VC, sender: self)
    }
    ...
    @IBAction func button_blockUserTapped(_ sender: Any) {
        ...
        if(self.lblBlockUser.text == Constants.strings.Block_User) {
            ...
            if (self.selectedBroadcaster != nil) {
                ...
                self.dataAccess.addNew_BlockedUser(blockedUser: b, delegate: self, senderTag: (self.selectedStream?.id)!)
            }
            else {
            ...
            // Going to remove follower
            if let b_user = Utilities.get_Required_BlockedUser_Object_fromList_using_blockedid(id: (self.selectedStream?.username)!) {
                self.dataAccess.remove_BlockedUser(blockedUser: b_user, resultDelegate: self, senderTag: (self.selectedStream?.id)!)
            }
        }
        else {
        ...
        if(self.selectedBroadcaster != nil) {
            ...
            QBRequest.createDialog(chatDialog, successBlock: {(response: QBResponse?, createdDialog: QBChatDialog?) in
                DispatchQueue.main.async {
                    ...
                    print(self.selectedStream?.callonly)
                    newQMChateVC.callOnly = "\(self.selectedStream!.callonly)"
                    newQMChateVC.both = "\(self.selectedStream!.bothmsgcall)"
...
//                }
    ...
    func get_Users_Success(_ users: [User]) {
        ...
        if selectedStream?.messageonly == true{
            button_Call.isHidden = true
            previousView.isHidden = false
            newView.isHidden = true
            
        }else if selectedStream?.callonly == true{
            
            button_Call.isHidden = false
            previousView.isHidden = true
            newView.isHidden = false
            
        }else if selectedStream?.bothmsgcall == true{
            previousView.isHidden = true
            newView.isHidden = true
            newView.isHidden = false
        }else{
        ...
        if (self.selectedBroadcaster?.skills == Constants.userSkillsType.client) {
            // self.rateViewContainer.isHidden = false
            self.button_appointment.isHidden = false
        } else {
    ...
    @objc func imageTapped(tapGestureRecognizer: UITapGestureRecognizer) {
        ...
        if (!(self.selectedBroadcaster?.link)!.isValidLinkedinLink) {
            Toast(text: "Invalid Linkedin profile path!", duration: Delay.short).show()
            return
        }
    ...
    func Add_Follower_ResponseSuccess(senderTag: Int, id: String)
    {
        ...
        Toast(text: "You started following \(String(describing: (self.selectedStream?.name)!))").show()
    ...
    func removeFollower_Delegate_Response(isSuccess: Bool , error: String, senderTag: Int, id: String)
    {
        ...
        if isSuccess
        {
            ...
            Toast(text: "You Unfollowed \(String(describing: (self.selectedStream?.name)!))").show()
    ...
    func Add_BlockedUser_ResponseSuccess(senderTag: Int, id: String)
    {
        ...
        Toast(text: "You blocked \(String(describing: (self.selectedStream?.name)!))").show()
    ...
    func Remove_BlockedUser_ResponseSuccess(senderTag: Int, id: String)
    {
        ...
        Toast(text: "You Unblocked \(String(describing: (self.selectedStream?.name)!))").show()
    ...
    @IBAction func followersButtonTapped(_ sender: Any)
    {
        ...
        self.performSegue(withIdentifier: Constants.Segues.BroadcastVC_to_FollowersList_VC, sender: self)
    }
    ...
    @IBAction func followingsButtonTapped(_ sender: Any)
    {
        ...
        self.performSegue(withIdentifier: Constants.Segues.BroadcastVC_to_FollowingsList_VC, sender: self)
    }
...
extension BroadCaster_ProfileVC: UITableViewDelegate, UITableViewDataSource
{
...
extension BroadCaster_ProfileVC{
